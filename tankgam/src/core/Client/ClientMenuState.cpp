...
ClientMenuState::ClientMenuState(Client& client, Renderer& renderer, Log& log, Net& net, MenuType menuType)
    : renderer{ renderer }, type{ menuType }
{
    ...
    case MenuType::MainMenu:
        {
            const auto mainListCallback = [this, &client, &renderer, &log, &net](size_t choice) -> void
            {
                switch (choice)
                {
                case 0:
                    client.pushState(std::make_shared<ClientConnectingState>(client, renderer, log, net,
